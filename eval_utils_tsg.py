...
def eval_split(model, crit, loader, eval_kwargs={}):
    ...
    while True:
        ...
        if data.get('labels', None) is not None and verbose_loss:
            ...
            tmp = [data['fc_feats'], data['att_feats'], data['labels'], data['masks'], data['cross_masks'],
                   data['enc_self_masks'], data['rela_seq']]
            ...
            with torch.no_grad():
                loss = crit(model(fc_feats, att_feats, labels, cross_masks,enc_self_masks,rela_seq, rela_len, attr_len, att_len), labels[..., 1:], masks[..., 1:]).item()
            loss_sum = loss_sum + loss
            loss_evals = loss_evals + 1

        # forward the model to also get generated samples for each image
        # Only leave one feature for each image, in case duplicate sample
        tmp = [data['fc_feats'], data['att_feats'], data['cross_masks'], data['enc_self_masks'], data['rela_seq']]
        tmp = [_ if _ is None else _.cuda() for _ in tmp]
        fc_feats, att_feats, cross_masks, enc_self_masks, rela_seq = tmp
        rela_len = data['rela_len']
        attr_len = data['attr_len']
        att_len = data['att_len']
        ...
        with torch.no_grad():
            ...
            seq, seq_logprobs = model(fc_feats, att_feats, cross_masks,enc_self_masks,rela_seq, rela_len, attr_len, att_len, opt=tmp_eval_kwargs, mode='sample')
            seq = seq.data
        ...
        if sample_n > 1:
            eval_split_n(model, n_predictions, loader, [fc_feats, att_feats, cross_masks, data], eval_kwargs)
