...
class CaptionModel(nn.Module):
    ...
    def beam_search(self, init_state, init_logprobs, rela_len, attr_len, att_len, *args, **kwargs):
        ...
        for t in range(self.seq_length + group_size - 1):
            for divm in range(group_size): 
                if t >= divm and t <= self.seq_length + divm - 1:
                    ...
                    if opt['caption_model'] == 'itransformer':
                        logprobs_table[divm], state_table[divm], out_tmp = self.get_logprobs_state(it.cuda(), *(args[divm] + [state_table[divm]]))
                    else:
                        logprobs_table[divm], state_table[divm] = self.get_logprobs_state(it.cuda(), *(
                                    args[divm] + [state_table[divm]]), rela_len, attr_len, att_len)
