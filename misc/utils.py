...
def decode_sequence_tt(ix_to_word, seq, end_index):
    N, D = seq.size()
    out = []
    for i in range(N):
        txt = ''
...
def save_checkpoint_new(opt, model, infos, optimizer, save_id, histories=None):
    # if checkpoint_path doesn't exist
    if not os.path.isdir(opt.checkpoint_path):
        os.makedirs(opt.checkpoint_path)
...
class StructureLosses(nn.Module):
    ...
    def forward(self, input, seq, data_gts):
        ...
        if self.opt.caption_model == 'ttransformer':
            mask = (seq != self.opt.word_end_ix).double()
        else:
            mask = (seq > 0).double()
...
class LanguageModelCriterion(nn.Module):
    ...
    def forward(self, input, target, mask, vocab = None):
        if target.ndim == 3:
            target = target.reshape(-1, target.shape[2])
            mask = mask.reshape(-1, mask.shape[2])
        # truncate to the same size
        target = target[:, :input.size(1)]
        mask =  mask[:, :input.size(1)].double()

        output = -input.gather(2, target.unsqueeze(2)).squeeze(2) * mask
        # if torch.isnan(output).any():
        #     a = -input.gather(2, target.unsqueeze(2))
        #     b = a.squeeze(2)
        #     c = b * mask
        # Average over each token
        # if vocab is not None:
        #     gt_sequence=decode_sequence_tt(vocab,target, mask)
        #     pred_sequence = decode_sequence_tt(vocab,input.argmax(dim=-1), mask)
        #
        #     print('gt: {0}\n'.format(gt_sequence))
        #     print('pred: {0}\n'.format(pred_sequence))

        out = torch.sum(output) / torch.sum(mask)
...
def kmeans_online(M, X, opt):
    dis = COS_dis(M, X)
    choice_points = torch.argmin(dis, dim=0)
    for k in range(opt.M_size):
        index =  torch.nonzero(choice_points == k).squeeze()
        if index.nelement() == 0:
            continue
        elif index.nelement()==1:
            x = X[index]
        else:
            x = torch.mean(X[index],dim=0)
        M[k] = opt.km_alpha * M[k] + (1 - opt.km_alpha) * x
    return M, choice_points
...
def COS_dis(M, X):
    ...
    # return K*N matrix for pairwise distance
    cosine_dis = 1 - torch.matmul(A_normalized.squeeze(), B_normalized.squeeze().transpose(0, 1))
