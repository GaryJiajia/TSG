...
def get_scores(data_gts, gen_result, opt):
    ...
    gen_result = gen_result.data.cpu().numpy()
    if opt.caption_model == 'ttransformer':
        for i in range(batch_size):
            end_index = np.where(gen_result[i]==opt.word_end_ix)[0][0]+1
            res[i] = [array_to_str(gen_result[i,0:end_index])]

        gts = OrderedDict()
        for i in range(len(data_gts)):
            gts[i] = []
            for j in range(len(data_gts[i])):
                if np.isin(opt.word_end_ix, data_gts[i][j]):
                    end_index = np.where(data_gts[i][j] == opt.word_end_ix)[0][0]+1
                else:
                    end_index = len(data_gts[i][j])
                gts[i].append(array_to_str(data_gts[i][j][0:end_index]))
